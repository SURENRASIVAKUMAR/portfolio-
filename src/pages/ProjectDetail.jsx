...
import VoiceChessDemo from '../components/VoiceChessDemo';
    ...
    <div style={{ backgroundColor: '#0f172a', color: '#fff', minHeight: '100vh', paddingBottom: '100px' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '60px 20px' }}>
        ...
        <motion.div 
          ...
          style={{ width: '100%', borderRadius: '16px', overflow: 'hidden', marginBottom: '60px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', backgroundColor: '#050814', position: 'relative' }}
        >
          {project.id === 3 ? (
            <div style={{ width: '100%', height: '500px', overflow: 'hidden', position: 'relative' }}>
              <iframe
                src="https://www.youtube.com/embed/9d8wWcJLnFI?autoplay=1&mute=1&controls=0&showinfo=0&rel=0&loop=1&playlist=9d8wWcJLnFI"
                style={{
                  position: 'absolute',
                  top: '50%',
                  left: '50%',
                ...
                title="Road Trip Background Video"
              ></iframe>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(5, 8, 20, 0.1)' }}></div>
            </div>
          ) : (
            <img src={project.image} alt={project.title} style={{ width: '100%', height: 'auto', maxHeight: '600px', objectFit: 'cover' }} />
          )}
        </motion.div>
        ...
        {/* Conditionally render Voice Chess Demo */}
        {project.id === 2 && (
